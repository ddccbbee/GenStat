...
class Datasets():
  ...
  def fet_features_dim(self):
    """
    :return: the dim of each kerne/feature/graph invariant
    """
    dim_ = []
    for element in self.featureList:
        # dim_.append(element[0].shape[-1] if len(element.shape) > 1 else 1)
        dim_.append(element[0].shape[-1])
    return dim_
  ...
  def get__(self,from_, to_, self_for_none, bfs=None, ignore_isolate_nodes = False):
      ...
      if bfs==None:
          ...
          if self.labels!=None:
              label= self.labels[from_:to_]
          X_s = None
          if X_s!=None:
              X_s = self.x_s[from_:to_]
          return self.adj_s[from_:to_], X_s, self.num_nodes[from_:to_], self.subgraph_indexes[from_:to_], graphfeatures,label
      ...
      return adj_s, x_s, num_nodes, subgraph_indexes, label

  def get__Numbers_of_coonented_componnets(self):
    components = []
    for adj in self.processed_adjs:
  ...
  def process(self,index,self_for_none, padded_to=None, bfs_max_length = None, ignore_isolate_nodes=True):
      ...
      if self_for_none:
          ...
          if max_num_nodes != num_nodes:
              adj_padded.setdiag(0)
              warnings.warn("this part amy have ptoblem")
          else:
              adj_padded.setdiag(0)
...
def list_graph_loader( graph_type, _max_list_size=None, return_labels=False, limited_to=None):
  ...
  if graph_type=="IMDBBINARY":
      data = dgl.data.GINDataset(name='IMDBBINARY',raw_dir="raw_dir/", self_loop=False)
      ...
      data = dgl.data.GINDataset(name='MUTAG', self_loop=False,raw_dir="raw_dir/")
      graphs, labels = data.graphs, data.labels
      for i, graph in enumerate(graphs):
          list_adj.append(csr_matrix(graph.adjacency_matrix().to_dense().numpy()))
          # list_x.append(graph.ndata['feat'])
      ...
      data = dgl.data.GINDataset(name='PTC', self_loop=False,raw_dir="raw_dir/")
      graphs, labels = data.graphs, data.labels
      for i, graph in enumerate(graphs):
          list_adj.append(csr_matrix(graph.adjacency_matrix().to_dense().numpy()))
          # list_x.append(graph.ndata['feat'])
      ...
      for i, graph in enumerate(data):
          ...
          list_x.append(None)
          # list_labels.append(labels[i].cpu().item())
  elif graph_type=="ogbg-ppa":
      # https://ogb.stanford.edu/docs/graphprop/
      from ogb.graphproppred import DglGraphPropPredDataset, collate_dgl
      d_name = "ogbg-ppa"  # ogbg-molhiv   'ogbg-code2' ogbg-ppa
  ...
  elif graph_type == "100_PVGAErandomGraphs":
      import randomGraphGen
      graphGen = randomGraphGen.GraphGenerator()
      for i in range(100):
          # graphs size
          # print(i)
          n = np.random.randint(low=20, high=200)
          # n = 20
          graph, graph_type = graphGen(n)
          list_x.append(None)
          list_adj.append(nx.adjacency_matrix(graph))
          list_labels.append(graph_type)
      ...
      for i in range(100):
          ...
          list_labels.append(graph_type)
      # graphs_to_writeOnDisk = [gr.toarray() for gr in list_adj]
      np.save('PVGAErandomGraphs.npy', list_adj, allow_pickle=True)
      np.save('PVGAErandomGraphs_labels.npy', list_labels, allow_pickle=True)
  elif graph_type == "PVGAErandomGraphs_dev":
      list_adj = list(np.load('data/PVGAErandomGraphs.npy', allow_pickle=True))
      list_x = [None for adj in list_adj]
      list_labels= list(np.load('data/PVGAErandomGraphs_labels.npy', allow_pickle=True))

  elif graph_type == "random_powerlaw_tree":
      list_adj = list(np.load('data/random_powerlaw_tree.npy', allow_pickle=True))
      list_x = [None for adj in list_adj]
      # list_labels= list(np.load('data/PVGAErandomGraphs_labels.npy', allow_pickle=True))
      ...
      #     n = np.random.randint(low=20, high=200)
      #     # n = 20
      #     graph, graph_type = graphGen(n, graph_type="random_powerlaw_tree")
      #     list_x.append(None)
      #     list_adj.append(nx.adjacency_matrix(graph))
      #     list_labels.append(graph_type)
      # # graphs_to_writeOnDisk = [gr.toarray() for gr in list_adj]
      # np.save('random_powerlaw_tree.npy', list_adj, allow_pickle=True)
      # np.save('PVGAErandomGraphs_labels.npy', list_labels, allow_pickle=True)
      ...
      for i in range(100000):
          ...
          graphGen = randomGraphGen.GraphGenerator()
          list_x.append(None)
          list_adj.append(nx.adjacency_matrix(graphGen(n)[0]))
      # graphs_to_writeOnDisk = [gr.toarray() for gr in list_adj]
      ...
      # np.save('mnist_600.npy', list_adj[:600], allow_pickle=True)
      # np.save('mnist.npy', list_adj, allow_pickle=True)
  elif graph_type=="mnist_01_percent":
      list_adj = []
      list_x = []
      import torch_geometric
      dataset_b = torch_geometric.datasets.MNISTSuperpixels(root="data/geometric")
      index = list(range(len(dataset_b.data.y)))[:600]
      random.shuffle(index)
      for i in index:  # len(dataset_b.data.y)
          in_1 = dataset_b[i].edge_index[0].detach().numpy()
          in_2 = dataset_b[i].edge_index[1].detach().numpy()
          valu = numpy.ones(len(in_2))
          adj = scipy.sparse.csr_matrix((valu, (in_1, in_2)), shape=(dataset_b[i].num_nodes, dataset_b[i].num_nodes))
      ...
      for i in range(len(dataset_b.data.y)):
          ...
          list_x.append(None)
      # random.shuffle(list_adj)
      # np.save('data/geometric/MoleculeNet/zinc_01.npy', list_adj[:int(len(list_adj)*.01)], allow_pickle=True)
      # np.save('data/geometric/MoleculeNet/zinc.npy', list_adj, allow_pickle=True)
...
def data_split(graph_lis, list_x=None, list_label=None):
    ...
    list_lanel_train = list_label_test=None
    if list_label!=None:
        list_lanel_train = list_label[0:int(0.8 * graph_test_len)]  # train
        list_label_test = list_label[int(0.8 * graph_test_len):]
    return  graph_train, graph_test, list_x_train , list_x_test,list_lanel_train, list_label_test
...
if __name__ == '__main__':
    ...
    # np.random.seed(10)
    path="DataSetSamples/"
    datasets = ["DD", "lobser", "MUTAG", "grid","triangular_grid","ogbg-molbbbp", "PTC", "QM9" ]
    for dataset in datasets:
        ...
        for i, G in enumerate(result[0][:20]):
            ...
            plotter.plotG(G, type=re.sub(r'[^a-zA-Z0-9]', '', result[-1][i]) + "_" + str(i), file_name = filename+"/rand_sa_"+str(i))
    ...
    for i, G in enumerate(result[0]):
        ...
        plotter.plotG(G, type=re.sub(r'[^a-zA-Z0-9]', '', result[-1][i][:14]) + "_" + str(i))
    # ----------------------------------------
    import plotter
