...
parser.add_argument('-v', dest="Vis_step", default=40000, help="model learning rate")
...
parser.add_argument('-ideal_Evalaution', dest="ideal_Evalaution" , default=True, help="if you want to comapre the 50%50 subset of dataset comparision?!", type=bool)
parser.add_argument("-desc_fun", dest="desc_fun" , default= [ "in_degree_dist", "HistogramOfCycles","HistogramOfRandomWalks","ReachabilityInKsteps"], help="the descriptor function which should be used", nargs='+',)
...
parser.add_argument('-arg_step_num"', dest="arg_step_num" , default=None, help="Number of Epochs", type=int)
parser.add_argument('-steps_of_reachability', dest="arg_steps_of_reachability" , default=None, help="Number of Epochs", type=int)
parser.add_argument('-max_size_of_cyc', dest="arg_max_size_of_cyc" , default=None, help="max graph cycle in the cycle kernel", type=int)
parser.add_argument('-scheduler_type', dest="scheduler_type" , default="None", help="OneCyle or CyclicLR, OR None", )
...
parser.add_argument('-LDP', dest="LDP" , default=False, help="this paprameter indicate if the graph generation should be under LDP, if LDP == True then graph statisics would be in_degree_dist and HistogramOfCycles, we have not implemented the  Random mechanism for the other statistics ")
parser.add_argument("-epsilon",dest="epsilon" , default=1.5,  help="The LDP parameter",type=float )
...
class kernel(torch.nn.Module):
    ...
    def __init__(self, **ker):
        ...
        if "HistogramOfRandomWalks" in kernel_set:
            ...
            self.ignoreCycles = ker.get("ignoreCycles")
    ...
    def RandWalkHist(self, adj, ApplyHis=True):
        feature_vec = []
        kth_walks = self.S_randomWalks(adj,self.num_of_steps,ignoreDiag=self.ignoreCycles)
        for h_,walk in enumerate(kth_walks):
            ...
            if ApplyHis:
                feature_vec.append(self.walks_hist[h_](walk[:,torch.triu_indices(walk.shape[-1], walk.shape[-1], offset=1)[0],torch.triu_indices(walk.shape[-1], walk.shape[-1], offset=1)[1]]))
            else:
    ...
    def S_randomWalks(self, adj, s=4, ignoreDiag=False ):
        """
    ...
    def ReachabilityInKsteps(self, adj,applyHist=True):
        ...
        for k_step in range(self.steps_of_reachability):
            ...
            if applyHist:
                ...
                re = self.ReachabilityHist[k_step](reached.sum(-1))
            else:
                re = reached.sum(-1)
            if self.normalized:
                re = re / adj.shape[-1]
...
kernel_ = kernel(kernel_type = [""],step_num = step_num, steps_of_reachability = steps_of_reachability, ignore_cycles=ignore_cycles)
...
if LDP==True:
    ...
    kernel_model = kernel(cyclesHistogram=cyclesHistogram, kernel_type = kernl_type, step_num = step_num, steps_of_reachability = steps_of_reachability,degree_hist = degree_hist,
                      ReachabilityInKsteps=None, max_size_of_cyc = max_size_of_cyc, HistogramOfRandomWalks = None, largest_graph_size=SubGraphNodeNum,normalized=data_Normalizer,ignore_cycles=ignore_cycles)
    ...
    for i_ in range(step_num):
        i = i_+1
        bin_center, binwith = _Binner.combineBinner(binners, randWalks[i_], num_of_bin, UpperBound=SubGraphNodeNum**i )
    ...
    for i_ in range(steps_of_reachability):
        bin_center, binwith = _Binner.combineBinner(binners, featureValuse[i_], num_of_bin, UpperBound=SubGraphNodeNum )
        reach_hist.append(Histogram( bin_center.to(device), binwith.to(device),MembershipFunction))
# --------------------------

    del k_steps, adj_list_tmp, kernel_
    kernel_model = kernel(cyclesHistogram=cyclesHistogram, kernel_type = kernl_type, step_num = step_num, steps_of_reachability = steps_of_reachability,degree_hist = degree_hist,
                      ReachabilityInKsteps=reach_hist, max_size_of_cyc = max_size_of_cyc, HistogramOfRandomWalks = HistogramOfRandomWalks, largest_graph_size=SubGraphNodeNum,normalized=data_Normalizer,ignore_cycles=ignore_cycles)
